...
namespace API_He_thong.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        [HttpPost("Create")]
        public async Task<IActionResult> PostUser(NguoiDung user)
        {
            if (user == null)
            {
        ...
        // PUT: api/User/Edit/5
        [HttpPut("Edit/{id}")]
        public async Task<IActionResult> EditUserById(int id, NguoiDung user)
        {
            if (user == null || id != user.nguoi_dung_id)
            {
                return BadRequest("Invalid user data.");
            }
