...
namespace API_He_thong.Controllers
{
    ...
    {
        private readonly IUngtuyen uS;

        public UngTuyenController(IUngtuyen us)
        {
            this.uS = us;
        }
        ...
        [HttpPost("Create")]
        public async Task<IActionResult> PostUser(UngTuyen user)
        {
            if (user == null)
            {
        ...
        // PUT: api/User/Edit/5
        [HttpPut("Edit/{id}")]
        public async Task<IActionResult> EditUserById(int id, UngTuyen user)
        {
            if (user == null || id != user.cong_viec_id)
            {
                return BadRequest("Invalid user data.");
            }
